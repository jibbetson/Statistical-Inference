...
**Variance** is a measure of the spread of the distribution. If $X$ is a random variable with mean $\mu$, the variance of $X$ is defined as  

$Var(X) = E[(X - \mu)^2] = E[X^2] - E[X]^2$.  

The rightmost equation is almost always used in practice. Thus the variance is the expected (squared) distance from the mean. Densities with a higher variance are more spread out than densities with a lower variance. The square root of the variance is called the **standard deviation**. The main benefit of working with standard deviations is that they have the same units as the data, whereas the variance has the units squared.  
